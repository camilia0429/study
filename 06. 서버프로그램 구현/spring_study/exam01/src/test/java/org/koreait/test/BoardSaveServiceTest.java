...
@Transactional // 테스트시에 추가된 데이터를 다시 롤백
public class BoardSaveServiceTest {
    ...
    @Test
    @DisplayName("필수항목(poster,subject,content) 검증, 검증 실패시 BoardValidationException 발생")
    void requiredFieldsTest() {
        /** poster 필수 검증(null, 빈값) */
        assertAll(
                () -> {
                    // poster가 null
                    boardData = getData();
    ...
    @Test
    @DisplayName("저장 전 데이터와 저장된 데이터의 일치 여부")
    void saveResultTest() {
        saveService.save(boardData);
    ...
    @Test
    @DisplayName("필수항목 검증(Bean Validation), 작성자, 제목, 내용 포함 여부")
